...
const Transactions2 = () => {
  ...
  const [timer, setTimer] = useState(() => {
    // Try to get the stored timer value from localStorage, default to 60 if not found
...
useEffect(() => {
  ...
  intervalRef.current = setInterval(() => {
    setTimer((prev) => {
      if (prev === 1) {
        addRandomAd(); // Add one ad when the timer reaches 1
        localStorage.setItem("timer", 60); // Reset timer in localStorage
        return 60; // Reset the timer
      } else {
        const newTimer = prev - 1;
        localStorage.setItem("timer", newTimer); // Save updated timer
        return newTimer;
      }
    });
  }, 1000);
...
useEffect(() => {
  ...
  const addRandomAd = () => {
    ...
    setOrders((prevOrders) => {
      // Check if the new order is already in the list based on orderNumber
      const isDuplicate = prevOrders.some(order => order.orderNumber === newOrder.orderNumber);

      if (isDuplicate) {
        console.log("Duplicate order detected, not adding:", newOrder);
        return prevOrders; // Return previous orders if duplicate
      }
     ...
     const startTimer = () => {
      if (!intervalRef.current) {
        intervalRef.current = setInterval(() => {
          addRandomAd();
        }, 300000); // Generate one ad every minute
      }
    };
